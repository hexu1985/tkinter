...
class Key(Button):
    def __init__(self, master, font=('arial', 8, 'bold'), 
        ...
        # apply(Button.__init__, (self, master), kw)    # python2 only
...
class Calculator(Frame):
    ...
    def doThis(self,action):
        # print '"%s" has not been implemented' % action    # python2 only
...
class Evaluator:
    ...
    def runpython(self, code):
        try:
            # return `eval(code, self.myNameSpace, self.myNameSpace)`   # python2 only
            return eval(code, self.myNameSpace, self.myNameSpace)
        except SyntaxError:
            try:
                # exec code in self.myNameSpace, self.myNameSpace       # python2 only
